...
bool lab5PitchShift(float *bufferIn) {
    ...
    int new_epoch_spacing;
    new_epoch_spacing = int(F_S/FREQ_NEW);
    // If voiced
    if (periodLen > 0) {
        ...
        int e_idx, cloest_e,  before, after,P_0 = 0;

        int minIdx = 1 , maxIdx = epochLocations.size()-1 ;
        while(newEpochIdx < 2*FRAME_SIZE){
            e_idx = findClosestInVector(epochLocations,newEpochIdx, minIdx, maxIdx);
            cloest_e = epochLocations[e_idx];
            before = epochLocations[e_idx-1];
            after = epochLocations[e_idx+1];
            P_0 = int((after - before)/2);
            float data[2*P_0+1];

            for(int i = 0; i<2*P_0+1;i++){
                data[i] = getHanningCoef(2*P_0+1,i)*bufferIn[cloest_e - P_0+i];
            }



        overlapAddArray(bufferOut,data,(newEpochIdx - P_0),(2*P_0-1));
